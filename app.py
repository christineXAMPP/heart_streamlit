...
st.markdown(
    ...
    /* uniform padding inside sidebar */
    section[data-testid="stSidebar"] .block-container {
        padding-left: 18px;
        padding-right: 18px;
    }

    /* make inputs full width in sidebar */
    ...
    /* two-column spacing inside sidebar rows */
    ...
    /* center the main visuals area horizontally */
    .centered-visuals {
        margin-left: 18px;
        margin-right: 18px;
    }
...
with c10:
    oldpeak = st.number_input("ST depression (oldpeak)", value=1.0, format="%.2f", step=0.1)
...
# ---------- Prediction function (session_state) ----------
def do_predict():
...
# ---------- Main layout: left margin, main, right container, right margin ----------
st.title("Heart Disease Predictor (Streamlit)")

left_margin, main_col, right_col, right_margin = st.columns([0.25, 3, 1.1, 0.25])
...
# Main column: centered charts with scroll container and horizontal margins
with main_col:
    st.markdown('<div class="scroll-container centered-visuals">', unsafe_allow_html=True)
